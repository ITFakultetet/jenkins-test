pipeline {
  ...
  stages {
    ...
    post {
      success {
        emailext body: "Dette er en mail fra Jenkins pipeline<p>Jenkins sier: <p> <b>Jobb:</b> ${env.JOB_NAME}<br><b>Byggnummer:</b>  ${env.BUILD_NUMBER} gikk bra!", subject: 'Jenkins-test - Ny versjon bygget!', to: 'terje@itfakultetet.no'
      ...
      failure {
       emailext body: "Dette er en mail fra Jenkins pipeline<p>Jenkins sier<p>: <br><b>Jobb</b>: ${env.JOB_NAME}<br><b>Byggnummer:</b>  ${env.BUILD_NUMBER} mislyktes!", subject: 'Bygging av Jenkins-test feilet', to: 'terje@itfakultetet.no'
